...
class RBF(keras.layers.Layer):
  ...
  def __init__(self, num_units, initializer):
    self.num_units = num_units
    self.initializer = initializer
    super(RBF, self).__init__()

  def build(self, input_shape):
    self.centers = self.add_weight(shape=(self.num_units, input_shape[1]), initializer=self.initializer, trainable=False) # TODO use custom initializer
    # print(self.centers)
